...
| `gh` CLI サブプロセス    | `src/token_source.rs` の `spawn_gh`                           | `gh auth token`。`TOKEN_RESOLVE_TIMEOUT` でタイムアウト。stderr はトークンを含みうるので破棄し終了コードだけ報告する (T-TOK004 が「stderr がログへ出ないこと」を assert) |

...
`Scout` holds `clock: Arc<dyn Clock>`, `rng: Arc<dyn Rng>`, and `token_source: Arc<dyn TokenSource>`. `token_source` is forwarded into `GitHubClient` on first `Scout::github()` call (`OnceCell` lazy init), so non-GitHub commands (`search`, `fetch`, `research`) never pay the `gh auth token` cost. `clock` and `rng` are Scout-wide rather than GitHub-specific, and reach their three destinations on different schedules: `GitHubClient` and `SlackClient` lazily through their own `OnceCell`s, `BraveClient` eagerly inside `ScoutBuilder::build`. All three client types carry `with_clock` / `with_rng` setters.

...
>
> **Note (2026-08-07, users.info 呼び出し失敗の cap との区別)**: `fetch_slack` の user 解決が `users.info` 呼び出し自体の失敗 (transport/API error) で raw ID にフォールバックしても、従来は `SlackUsersCapped` (件数超過による cap) と区別がつかなかった。呼び出し側 AI エージェントが raw ID の原因を判別できるよう `SlackLookupFailed` variant 追加で**計 14 variants**。`SlackClient::prefetch_users` が in-cap の `users.info` 呼び出しのうち 1 件でも失敗すれば `SlackFetchOutcome::lookups_failed` を立て、`fetch_slack` (`src/tools/query.rs`) が cap 系 3 variant と同じ形で preamble note (`"Some user or channel lookups failed, so those authors and mentions show raw IDs."`) を組み立てて `degradation.push` する。この variant も callsite で note text を直接構築するため `unwrap_or_degraded` を経由せず、`label()` は既存の `"resource"` 群にそのまま合流する。よって `unwrap_or_degraded` 経由で meaningful label を持つのは引き続き `*FetchFailed` の 3 variants で不変。追加は serde additive (新 variant は `degraded_reasons` の skip-if-empty で feature-detect 可能) のため既存 JSON consumer に無影響。
>

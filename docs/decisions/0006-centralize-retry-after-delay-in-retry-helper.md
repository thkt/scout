...
`tools/errors.rs` has a parallel duplication: `From<BraveError>` and `From<GitHubError>` for `ScoutError` share a structurally identical 4-arm transient cluster (RateLimited / Server-or-Api-5xx / Network / Network-is-timeout). This ADR's scope includes consolidating that pattern via three small helpers (`transient_with_retry_hint`, `transient_with_network_hint`, `timeout_with_retry_hint`) rather than via macro or trait. Same reasoning as the main decision: helpers are type-safe, debuggable, and avoid macro indirection.

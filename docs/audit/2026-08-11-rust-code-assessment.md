...
対象: scout v2.5.0 (edition 2024, rust-version 1.97.1)
測定日: 2026-08-17/基準 commit c0499fd

他リポジトリを scout の書き方に寄せてリファクタリングするための参照資料として書いた。scout 自身の改善点は E 節と F 節に分けてある。初版 (2026-08-10) は F 節の指摘を 3 commit で修正した時点のもので、2026-08-11 に実装ファイル 50 本を 1 本ずつ読み切って D 節を追加した。2026-08-17 に v2.5.0 (33 ファイル +4,913 行) を反映して実測値と A-C 節、E 節を測り直し、file:line 参照から行番号を外した。行番号はこの文書の外の変更で古くなり、古いことが誰にも見えない。関数名と型名は `ugrep -F 'fn <name>'` で一意に引ける。D 節と F 節は 2026-08-11 時点の調査記録なので、当時の記述のまま残す。
...
| 総行数                                            | 29,176                                                                                                                                                                                                                                     | `wc -l`                                                                                                                                            |
| src の実装ファイル                                | 50 ファイル / 15,842 行                                                                                                                                                                                                                    | ファイル名が `*tests.rs` / `*_tests.rs` でないもの。26 ファイルが inline `mod tests` を含み、`test_support.rs` と `test_helpers.rs` もこの側に入る |
| src のテスト専用ファイル                          | 45 ファイル / 11,502 行                                                                                                                                                                                                                    | ファイル名が `*tests.rs` / `*_tests.rs` のもの                                                                                                     |
| `tests/` ディレクトリ                             | 4 ファイル / 1,832 行                                                                                                                                                                                                                      | 統合テスト                                                                                                                                         |
| テスト数                                          | 854 passed (4 leaky) / 1 skipped                                                                                                                                                                                                           | `cargo nextest run --all-features`                                                                                                                 |
| lint 抑制                                         | 19。内訳は `#[expect(...)]` 7、`#[allow(clippy::needless_pass_by_value)]` 5 (`fetch/converter.rs`、htmd の trait bound 由来)、`#[cfg_attr(not(feature = "js-rendering"), allow(...))]` 6、`#![allow(dead_code)]` 1 (`tests/common/mod.rs`) | `grep -E '#!?\[(cfg_attr\(.*)?(allow\|expect)\('`。`#\[allow` だけの grep は cfg_attr 経由の 6 件も inner attribute も拾わない                     |
| 本番経路の `unwrap()` / `expect()`                | 2 (`lib.rs` 静的 directive、`envelope.rs` infallible Serialize) — 両方に理由コメントあり                                                                                                                                                   | 全 362 hit から、`#[cfg(test)]` 配下と `test_support.rs` / `test_helpers.rs` と doc comment 内の言及を除いた残り                                   |
| clippy `--all-targets --all-features -D warnings` | exit 0                                                                                                                                                                                                                                     | 実行済み                                                                                                                                           |
| clippy deny                                       | 13 (前版から変化なし。`Cargo.toml` の `[lints.clippy]` セクションのみ。`[lints.rust]` の `unsafe_code` はここに含めない)                                                                                                                   | —                                                                                                                                                  |
| `unsafe`                                          | `unsafe_code = "forbid"` (`[lints.rust]`)                                                                                                                                                                                                  | —                                                                                                                                                  |
| 到達できない `pub`                                | 0。`unreachable_pub = "deny"`                                                                                                                                                                                                              | clippy が exit 0                                                                                                                                   |
| テスト ID `[T-XXX]`                               | 出現 774 / 重複除去後 774 (重複定義なし)                                                                                                                                                                                                   | `grep -o` と `sort -u` の両方                                                                                                                      |
| 実装コード中の DR/ADR 参照                        | 142 箇所                                                                                                                                                                                                                                   | `grep -o 'ADR-[0-9]\{4\}\|DR-[0-9]\{4\}'` を `src/` に対して。`tests/` を足すと 157                                                                |
| Decision Record                                   | 27 件 (全て accepted)                                                                                                                                                                                                                      | `docs/decisions/` の `status:` 行                                                                                                                  |

前版が本番経路の `unwrap()`/`expect()` を 3 と数えたのは誤りだった。3 件目に挙げていた `rng.rs` の `SeededRng` は `#[cfg(test)] impl Rng for SeededRng` の中にあり、本番ビルドに残らない。実装ファイル名で絞る手順は inline の `#[cfg(test)]` ブロックを取りこぼす。

13 個の clippy deny を宣言し、抑制 19 個で通っている。6 個は `js-rendering` 無効ビルドで未使用になるコードを黙らせる `cfg_attr` 経由で、feature gate の副作用を消す定型。7 個は `#[expect]` で、抑制が不要になった日にそれ自体が警告として報告される。残る 5 個は `fetch/converter.rs` の `#[allow(clippy::needless_pass_by_value)]` で、Cargo.toml が deny しているのと同じ lint を打ち消している。理由は外部由来で、htmd の `ElementHandler` blanket impl が `Fn(&dyn Handlers, Element)` しか受けないため `&Element` にすると `add_handler` の bound を満たさない。5 箇所とも根拠コメントが付く。`#[expect]` との観測上の違いは、htmd が署名を変えて制約が消えた日に `#[expect]` なら警告が出て `#[allow]` なら黙って残る点にある。
...
`envelope.rs` が `ErrorCode` enum を 1 つ持ち、`exit_code()` と `is_retryable()` の両方をそこから導出する。`ScoutError::new` は `retryable` を引数で受けず `kind.is_retryable()` で決める (`tools/errors.rs`)。呼び出し側が `retryable: true` を書ける余地が無いので、JSON 契約と終了コードが構造的にずれない。

`classify.rs` の `from_http_status` は「3 つのバックエンドが HTTP status から独自に導出していて、実際に食い違っていた (GitHub 408 が DataError、Brave 404 が DataError)」という履歴をコメントに残したうえで 1 つのテーブルに統合している。逸脱したいバックエンドは委譲アームの手前に自分のアームを足す形で、逸脱が diff に見える。

シグナル由来の終了コードは別マッピングになっている。`signals.rs` の `InterruptSignal::exit_code()` が SIGINT → 130/SIGTERM → 143 を返す (POSIX の 128 + シグナル番号)。エラー分類とは由来が違うので統合していない。移植時もこの分離は保つ。

移植コスト: 小。enum + 2 メソッド。

### A-2. リトライ / バックオフの一元化 (DR-0006)

`retry.rs` (161 行) が全バックエンドのリトライ判断を持つ。
...
テストの doc comment 先頭に `[T-<PREFIX><NNN>]` を書き、DR がその ID を引用してテストを名指しする。規約自体が `test_support.rs` の crate doc に書かれている。ブラケット付きは定義、ブラケット無しは引用、という区別まで決めてある (grep で定義が一意に引ける)。prefix は「テスト対象」を指すので 1 prefix が複数ファイルに跨る。番号は prefix 内で一意で、ファイル単位ではない。

これで「この決定を守っているテストはどれか」が双方向に辿れる。774 ID/142 参照が実際に動いている。重複採番はリポジトリ自身のテスト (`test_support::scan_test_id_violations`) が検出するので、採番ミスは CI で止まる。
...
`ssrf.rs` は URL の userinfo をログ出力時に落とす `RedactedLogUrl<'a>` を持ち、パース失敗時は `[redacted-url]` に倒す (fail closed)。

移植コスト: 小。型を 1 つ作るだけ。

### A-6. `--help` を AI エージェント向けの契約書として書く

`lib.rs` の `after_help` に終了コード表、環境変数、チューニング用 env var の範囲まで全部載せる。さらにその内容をテストが pin している (`root_help_contains_exit_codes_and_environment`, `root_help_lists_scout_tuning_env_vars`)。`--version` 実行時に「coding agent なら `--help` を読め」というヒントを stderr に出す (`AGENT_HELP_HINT`)。
...
| `TokenSource` | `GhCliSource`      | `StaticTokenSource`                        | `token_source.rs` (223 行) |
...
移植コスト: 小。ファイル 4 つで 400 行未満。

### B-2. 検証済みを型で表す `ValidatedUrl`

`ssrf.rs`。`ValidatedUrl` の唯一のコンストラクタが `ssrf_check` で、下流 (`download`, `reqwest::Client::get`) は `&ValidatedUrl` しか受け取らない。SSRF チェックを飛ばす経路がコンパイル時に存在しない。
...
`RuntimeConfig::from_env_with<F: Fn(&str) -> Result<String, VarError>>` (`tools/config.rs`)。テストが `unsafe { env::set_var }` を使わずに parse 失敗と範囲外を全部通せる。`unsafe_code = "forbid"` と両立させるための設計だが、env をプロセスグローバル状態として触らないので並列テストが壊れないという利点が本体。

`detect_egress_mode(env: &HashMap<String, String>)` も同じ形 (データを受けてデータを返す)。

`VarError::NotUnicode` を「未設定」でなく「設定済みだが不正」として扱う判断が `read_env_raw` にある。設定したつもりの値が黙ってデフォルトに落ちない。

移植コスト: 小。

### B-5. degraded (部分失敗) の表現

`envelope.rs` の `Degradation` はフィールドを private にし、`push(message, reason)` だけを公開する。`notes[i]` と `reasons[i]` の対応がずれた状態を構築できない。`CommandOutput` も `degraded: false` と非空 `notes` の組み合わせを作れない。

`DegradedReason` は enum で JSON に `SCREAMING_SNAKE_CASE` で出る。呼び出し側は自由文の notes をパースせずに失敗モードを判別できる。

型で守れるのは「envelope の中で対応がずれないこと」までで、「通知が利用者に届くこと」は別問題になる (D-4)。DR-0010 と実装の食い違いは #432 で解消した。
...
`ScoutBuilder` は `github_endpoint`/`slack_endpoint` を `#[cfg(test)]` フィールドとして持ち、`build()` の中で `#[cfg(test)]` ブロックが `OnceCell` を事前に埋める (`tools/builder.rs`)。DR-0008 で意図的に選んだ設計で、`with_*` セッターも全て `#[cfg(test)]`。
...
`tools/config.rs` の `DEFAULT_GITHUB_TIMEOUT_SECS` は 12 行のコメントで、180 秒という値が「最も重い repo-overview の happy path を通す」「全タイムアウト時のリトライ予算 279 秒を下回る」という 2 つの境界から決まったことと、そのトレードオフを書いている。`classify.rs` も同様に「なぜ 1 箇所なのか」を過去の食い違い実例で説明する。
...
この粒度が全ファイルに行き渡っているわけではない。`fetch/converter.rs` は 3,177 行で、実装 1,016 行に inline `mod tests` が 2,161 行付く。2 番目に大きい実装ファイル (`test_support.rs` 900 行) の 3.5 倍で、1 ファイルとして読み切れる大きさを超えている (E-4)。分割粒度を移植するときは、この反例のほうが起きやすい姿だと見ておく。

小規模リポジトリにそのまま持ち込むとディレクトリが薄いファイルで埋まる。何を単位に分けるかを先に決めてから移す。scout の単位は関心で、行数ではない。実測ではテスト専用ファイルが中央値 156 行で最大 1,058 行、`fetch/cdp/launch/` 配下の 4 ファイルは 21 行から 93 行まで開く。規則としての本文は `.claude/rules/CONVENTIONS.md` にある (git 追跡外のローカル文書)。

---

## D. 欠陥の型 — 他リポジトリでも同じ探し方が使える

2026-08-11 の調査記録。実装ファイル 50 本を 1 本ずつ読んで 29 commit を積んだ結果、見つかった欠陥は 5 つの型に収まった。型ごとに「どう探すか」を書く。個別の修正内容より、この探し方のほうが移植価値が高い。
...
| `apply_line_range` (`github/helpers.rs`) の行番号カラム | 幅 5 固定で、10 万行 (10MB 上限に対して約 1MB) から桁が溢れる                                  |
...
- **数を文書に書く前に、grep の pattern が覆う範囲と文中の名詞を突き合わせる。** `#[allow]` を数える pattern は `#![allow]` にも `#[expect]` にもマッチしない。「抑制」という名詞はその 3 つ全部を指す。同じ型の誤りが本文書自身で再発した (2026-08-17 の測り直しで、本番経路の `unwrap()` 3 件が実は 2 件だと判明。ファイル名で絞る手順は inline の `#[cfg(test)]` ブロックを取りこぼす)
...
`tools/config.rs` が 5 フィールド分ほぼ同形の if を並べる。フィールド名が `tracing` の構造化ログのキーになっているので、ループで畳むとキーが文字列になり、`info!` のフィールド名の静的性が失われる。マクロで畳めるが、5 個のためにマクロを増やすのは割に合わない。

判断: 現状維持でよい。フィールドが 8-10 個に増えたら宣言的マクロを検討する境界。

### E-2. `with_clock` / `with_rng` の 4 重複 — 決着済み、追跡先は閉じている

`github.rs`/`brave/client.rs`/`slack/client.rs`/`tools/builder.rs` の 4 箇所に同形の `with_clock`/`with_rng` がある (計 8 メソッド)。共通化 (ClientCommon 化、DRY-02) は critic-design の実測で棄却済み。ADR-0007 の Reassessment Trigger が根拠にならない (ADR-0004 が ADR-0007 に先行する) と実証されたため、新 DR の起草が着手条件になっている。

その着手条件は #310 の Backlog candidates に書かれているが、#310 は closed で、ClientCommon/DRY-02 を引き継ぐ open issue は無い。条件付きの判断が閉じた issue の中だけに残っている状態なので、再検討するなら本文書か新 issue へ移す必要がある。

他リポジトリへ移すときは、この重複を「直っていない」と読まないこと。3 クライアントが同じ trait を実装していないのは、共有部分を切り出すコストが利得を上回るという判断の結果。

### E-3. `api_get_once` の二重パース — 現状維持

`slack/client.rs` は本文を `serde_json::Value` へ 1 回、`from_value` で目的の型へもう 1 回パースする。`ok: false` を目的型の deserialize より先に判定するために必要な形で、`#[serde(flatten)]` で 1 回に畳むと、`ok: false` かつ目的型に合わない本文が `Api` ではなく `Decode` に落ちる (エラー分類が変わる)。1 レスポンスは `MAX_API_RESPONSE_BYTES` (1MiB) で上限があるので、得るものより失うもののほうが大きい。

### E-4. `fetch/converter.rs` が 3,177 行 — 2026-08-17 に新規計上

実装 1,016 行に inline `mod tests` が 2,161 行付き、実装ファイルの中で 2 番目に大きい `test_support.rs` (900 行) の 3.5 倍になっている。増分の出所は #373 (fast_html2md → htmd の置き換え) で、表/コードブロック/pre の変換分岐ごとに回帰テストを積んだ結果。テストの量そのものは置き換えの対価として妥当だが、C-3 が挙げる「関心ごとに分けてあるので 1 ファイルは読み切れる」という scout の分割粒度からは外れている。

判断は未着手。他のテスト専用ファイル (`*_tests.rs`) と同じ形へ出すのが既存の分割規約に沿う。テスト ID の並びは表, pre とフェンス, リンクとアンカー, script と style の抑制, frontmatter, リストの 6 群に分かれるので、切り出す単位を先に決めないとファイルが 6 本に割れる。
...
- 英文の中に日本語 1 語: `classify.rs` の `// 退避: 1xx/3xx reaching an error path is not...`

全 121 行を英語へ揃えた。`退避` は ADR-0011 が Unknown スロットに与えた設計語彙で、`envelope.rs` が既に `retreat slot` と英語で書いていたため、そちらへ寄せて 8 箇所を統一した。
...
clippy の 13 deny リスト (`Cargo.toml` の `[lints.clippy]`) は 2.9 万行の CLI では快適に通るが、大規模アプリに一括で入れると既存コードが大量に落ちる。`needless_pass_by_value` と `cast_possible_truncation` はとくに影響範囲が広い。scout 自身も `needless_pass_by_value` を外部クレートの trait bound のために 5 箇所で `#[allow]` している。段階的に 1 つずつ deny に上げる形が現実的。
